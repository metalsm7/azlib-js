...
class AZSql {
    ...
    hasReturnParameters() {
        return this._return_parameters !== null && this._return_parameters instanceof azdata_1.AZData && this._return_parameters.size() > 0;
    }
    setResults(rows) {
        this._results = rows;
        return this;
    ...
    async closeAsync() {
        ...
        switch ((_a = this._option) === null || _a === void 0 ? void 0 : _a.sql_type) {
            case AZSql.SQL_TYPE.MYSQL:
                await this._sql_connection.end();
                break;
            case AZSql.SQL_TYPE.SQLITE:
                await new Promise((resolve, reject) => {
    ...
    async getAsync(query_or_id, param_or_id, return_param_or_id, is_sp) {
        ...
        // return rtn_val;
        const res = await this.getDataAsync(query_or_id, param_or_id, return_param_or_id, is_sp);
        if (res === null)
            return null;
        let rtn_val = null;
        const keys = Object.keys(res);
        keys.length > 0 && (rtn_val = res[keys[0]]);
        return rtn_val;
    }
    async getDataAsync(query_or_id, param_or_id, return_param_or_id, is_sp) {
        ...
        // return rtn_val;
        const res = await this.getListAsync(query_or_id, param_or_id, return_param_or_id, is_sp);
    ...
    async getListAsync(query_or_id, param_or_id, return_param_or_id, is_sp) {
        ...
        return this.hasResults() ? this.getResults() : new Array();
    }
                        ...
                        if (is_prepared) {
                            ...
                            if (err) {
                                ...
                                if (Array.isArray(rows)) {
                                    ...
                                    rtn_val = this.getIdentity() ? header.insertId : header.affectedRows;
                                    //
                                    this.inTransaction && this.addTransactionResult(rtn_val);
                                }
                                //
                                if (this.getIsStoredProcedure()) {
                                    const return_query = this.getReturnQuery();
                                    if (return_query !== null) {
                                        const [r_res, r_err] = await this._sql_connection.execute(return_query)
                                            .then((res) => {
                                            return [res, null];
                                        })
                                            .catch(async (err) => {
                            ...
                            const [res, err] = await this._sql_connection.query(query, params)
                                .then((res) => {
                                return [res, null];
                            })
                                .catch(async (err) => {
                        ...
                        if (is_prepared) {
                            ...
                            if (err) {
                                ...
                                if (Array.isArray(res)) {
                                    ...
                                    rtn_val = this.getIdentity() ? res.lastID : res.changes;
                                }
                            }
                        }
                        else {
...
(function (AZSql) {
    ...
    class Basic extends AZSql.BQuery {
        ...
        async doSelectAsync(select = '*') {
            ...
            // typeof res.rows !== 'undefined' && (rtn_val = res.rows);
            // return rtn_val;
            return await this._azsql.getListAsync(this.getQuery(AZSql.BQuery.CREATE_QUERY_TYPE.SELECT), this.getPreparedParameters());
        }
        async doInsertAsync(_get_identity = false) {
            if (typeof this._azsql === 'undefined') {
            ...
            // return rtn_val;
            const cur_identity = this._azsql.getIdentity();
            _get_identity && this._azsql.setIdentity(true);
            ...
            this._azsql.setIdentity(cur_identity);
            return rtn_val;
        }
        async doUpdateAsync(_require_where = true) {
            ...
            const cur_identity = this._azsql.getIdentity();
            this._azsql.setIdentity(false);
            const rtn_val = await this._azsql.executeAsync(this.getQuery(AZSql.BQuery.CREATE_QUERY_TYPE.UPDATE), this.getPreparedParameters());
            this._azsql.setIdentity(cur_identity);
            return rtn_val;
        }
        async doDeleteAsync(_require_where = true) {
            ...
            const cur_identity = this._azsql.getIdentity();
            this._azsql.setIdentity(false);
            const rtn_val = await this._azsql.executeAsync(this.getQuery(AZSql.BQuery.CREATE_QUERY_TYPE.DELETE), this.getPreparedParameters());
            this._azsql.setIdentity(cur_identity);
            return rtn_val;
        }
    }
