...
class AZSql {
    ...
    async getListAsync(query_or_id, param_or_id, return_param_or_id, is_sp) {
        const res = await this.executeAsync(query_or_id, param_or_id, return_param_or_id, is_sp);
        if (typeof res.rows === 'undefined' && typeof res.err !== 'undefined')
            throw res.err;
        let rtn_val = new Array();
...
(function (AZSql) {
    ...
    class Basic extends AZSql.BQuery {
        ...
        async doSelectAsync(select = '*') {
            ...
            const res = await this._azsql.executeAsync(this.getQuery(AZSql.BQuery.CREATE_QUERY_TYPE.SELECT), this.getPreparedParameters());
            if (typeof res.rows === 'undefined' && typeof res.err !== 'undefined')
                throw res.err;
            let rtn_val = new Array();
            typeof res.rows !== 'undefined' && (rtn_val = res.rows);
        ...
        async doUpdateAsync(_require_where = true) {
            ...
            return await this._azsql.setIdentity(true).executeAsync(this.getQuery(AZSql.BQuery.CREATE_QUERY_TYPE.UPDATE), this.getPreparedParameters());
        }
        async doDeleteAsync(_require_where = true) {
            ...
            return await this._azsql.setIdentity(true).executeAsync(this.getQuery(AZSql.BQuery.CREATE_QUERY_TYPE.DELETE), this.getPreparedParameters());
        }
    }
    AZSql.Basic = Basic;
